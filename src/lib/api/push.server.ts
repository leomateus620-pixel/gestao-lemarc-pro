type PushEventType = "service_order_assigned" | "service_order_finished";
...
export async function deliverPush({ userIds, eventType, title, body, serviceOrderId = null, data = {} }: DeliveryInput) {
  ...
  if (error) { console.error("[push] Falha ao buscar dispositivos", error.message); return; }
  const targets = (devices ?? []) as { id: string; user_id: string; token: string }[];
  if (targets.length === 0) {
    await Promise.all(ids.map((userId) => insertLog(sb, { event_type: eventType, channel: "fcm", user_id: userId, service_order_id: serviceOrderId, title, body, status: "skipped_no_token", metadata: data })));
    return;
  }

  let accessToken: string;
  let account: FcmServiceAccount;
  try { account = readServiceAccount(); accessToken = await createGoogleAccessToken(account); } catch (cause) {
    const message = cause instanceof Error ? cause.message : "Falha de autenticação FCM";
    await Promise.all(targets.map((target) => insertLog(sb, { event_type: eventType, channel: "fcm", user_id: target.user_id, service_order_id: serviceOrderId, title, body, fcm_token_suffix: tokenSuffix(target.token), status: "failed", error: message, metadata: data })));
    return;
  }
  ...
  await Promise.all(targets.map(async (target) => {
    try {
      ...
      if (response.ok) {
        ...
        await insertLog(sb, { event_type: eventType, channel: "fcm", user_id: target.user_id, service_order_id: serviceOrderId, title, body, fcm_token_suffix: tokenSuffix(target.token), fcm_message_id: result.name ?? null, status: "sent", metadata: data });
      ...
      await insertLog(sb, { event_type: eventType, channel: "fcm", user_id: target.user_id, service_order_id: serviceOrderId, title, body, fcm_token_suffix: tokenSuffix(target.token), status: unregistered ? "unregistered" : "failed", error: `FCM [${response.status}]: ${responseText}`, metadata: data });
      if (unregistered) await sb.from("push_devices").update({ revoked_at: new Date().toISOString() }).eq("id", target.id);
    } catch (cause) {
      await insertLog(sb, { event_type: eventType, channel: "fcm", user_id: target.user_id, service_order_id: serviceOrderId, title, body, fcm_token_suffix: tokenSuffix(target.token), status: "failed", error: cause instanceof Error ? cause.message : "Falha ao enviar push", metadata: data });
    }
  }));
}

export async function notifyAdminsOfFinishedOrder(input: { serviceOrderId: string; orderNumber: number; body: string }) {
  const sb = await getServerClient();
  const { data, error } = await sb.from("user_roles").select("user_id").eq("role", "admin");
  if (error) { console.error("[push] Falha ao buscar administradores", error.message); return; }
  await deliverPush({
