...
function readServiceAccount(): FcmServiceAccount {
  ...
  if (!account.client_email || !account.private_key || !account.project_id) {
    throw new Error("Credencial FCM incompleta.");
  }
  return account;
}
    ...
    await Promise.all(
      targets.map((target) =>
        insertLog(supabase, {
          ...
          title,
          body,
          fcm_token_suffix: tokenSuffix(target.token),
          status: "failed",
          error: message,
          metadata: data,
        }),
    ...
    targets.map(async (target) => {
      try {
        ...
        if (response.ok) {
          ...
          await supabase.from("push_devices").update({ last_seen_at: new Date().toISOString() }).eq("id", target.id);
          return;
        }
        ...
        await insertLog(supabase, {
          ...
          error: `FCM [${response.status}]: ${responseText}`,
          metadata: data,
        });
        if (unregistered) {
          await supabase.from("push_devices").update({ revoked_at: new Date().toISOString() }).eq("id", target.id);
        }
      } catch (cause) {
        await insertLog(supabase, {
          event_type: eventType,
          ...
          fcm_token_suffix: tokenSuffix(target.token),
          status: "failed",
          error: cause instanceof Error ? cause.message : "Falha ao enviar push",
          metadata: data,
        });
      }
    }),
...
export const registerPushDevice = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { error } = await (context.supabase.from("push_devices") as any).upsert(
      { token, user_id: context.userId, platform: data.platform ?? "web", user_agent: data.userAgent ?? null, last_seen_at: new Date().toISOString(), revoked_at: null },
      { onConflict: "token" },
    );
...
export const listNotificationDeliveryLog = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data, context }) => {
    ...
    let query = sb
      ...
      .select("id, created_at, event_type, user_id, service_order_id, title, body, fcm_token_suffix, fcm_message_id, status, error, metadata")
