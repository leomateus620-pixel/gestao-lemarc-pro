...
import { notifyAdminsOfFinishedOrder } from "@/lib/api/push.functions";
...
function normalizeFinancials(row: any): OrderFinancials | null {
  ...
  return {
    ...
    displacement_decided: row.displacement_decided === true,
...
export const finalizeServiceOrder = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    validateInput(data);
    const sb = context.supabase as any;
    const { data: previousOrder, error: previousOrderError } = await sb
      .from("service_orders")
      .select("status, number, client:clients(name, unit), client_unit:client_units(name), technician:technicians(full_name)")
      .eq("id", data.order_id)
      .maybeSingle();
    ...
    if (previousOrder?.status !== "finished" && previousOrder?.status !== "approved") {
      const client = Array.isArray(previousOrder?.client) ? previousOrder.client[0] : previousOrder?.client;
      const unit = Array.isArray(previousOrder?.client_unit) ? previousOrder.client_unit[0] : previousOrder?.client_unit;
      const technician = Array.isArray(previousOrder?.technician) ? previousOrder.technician[0] : previousOrder?.technician;
      try {
        await notifyAdminsOfFinishedOrder({
          serviceOrderId: data.order_id,
          orderNumber: Number(previousOrder?.number ?? 0),
          body: `${client?.name ?? "Cliente não informado"} · ${unit?.name ?? client?.unit ?? "Unidade não informada"} · ${technician?.full_name ?? "Técnico não informado"}`,
        });
      } catch (pushError) {
        console.warn("[push] Falha ao notificar administradores", pushError);
      }
    }
    return { ok: true };
  });
