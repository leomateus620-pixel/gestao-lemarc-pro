    ...
    <section className="lemarc-wizard-card space-y-4 p-5 sm:p-6">
      ...
      <div className="overflow-x-auto rounded-xl border border-white/10">
        <table className="w-full min-w-[820px] text-left text-xs">
          <thead className="bg-white/[0.05] text-[10px] uppercase tracking-wider text-slate-400"><tr><th className="px-3 py-3">Data</th><th className="px-3 py-3">Evento</th><th className="px-3 py-3">Destinatário</th><th className="px-3 py-3">OS</th><th className="px-3 py-3">Status</th><th className="px-3 py-3">Token</th><th className="px-3 py-3">Erro</th></tr></thead>
          <tbody className="divide-y divide-white/[0.07] text-slate-200">
            {isLoading ? <tr><td colSpan={7} className="px-3 py-8 text-center text-slate-400">Carregando histórico...</td></tr> : data.length === 0 ? <tr><td colSpan={7} className="px-3 py-8 text-center text-slate-400">Nenhuma tentativa encontrada.</td></tr> : data.map((row) => <tr key={row.id}><td className="whitespace-nowrap px-3 py-3">{new Date(row.created_at).toLocaleString("pt-BR")}</td><td className="px-3 py-3">{eventLabel(row.event_type)}</td><td className="max-w-[180px] truncate px-3 py-3 font-semibold">{row.recipient}</td><td className="px-3 py-3 font-semibold">{row.service_order_id ? row.service_order_id.slice(0, 8) : "—"}</td><td className="px-3 py-3">{statusLabel(row.status)}</td><td className="px-3 py-3 font-mono text-slate-400">{row.fcm_token_suffix ? `…${row.fcm_token_suffix}` : "—"}</td><td className="max-w-[260px] truncate px-3 py-3 text-rose-200">{row.error ?? "—"}</td></tr>)}
          </tbody>
        </table>
      </div>
    </section>
