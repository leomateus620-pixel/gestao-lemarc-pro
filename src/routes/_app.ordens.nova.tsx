...
function NovaOS() {
  ...
  return <AppShell title="Nova ordem de serviço" back>
    {created ? <GlassCard className="lemarc-hero-gradient mt-2 p-5 text-center"><ClipboardCheck className="mx-auto text-primary" size={34}/><h1 className="mt-3 font-display text-2xl font-black text-foreground">OS criada</h1><p className="mt-2 text-sm text-muted-foreground">Ordem mockada pronta para acompanhamento.</p><div className="mt-4"><PrimaryCTA to="/ordens/o-1042" icon={ClipboardCheck}>Abrir detalhe</PrimaryCTA></div></GlassCard> : <Stepper steps={steps} current={step} />}

    {!created && <div className="mt-5 space-y-3">
    ...
    {!created && <div className="sticky bottom-24 z-20 mt-6 flex gap-2 pb-2">{step > 0 && <Button variant="secondary" onClick={() => setStep((s) => s - 1)} className="h-12 flex-1 bg-secondary text-foreground hover:bg-secondary/80">Voltar</Button>}<div className="flex-[2]"><PrimaryCTA onClick={next} icon={step === steps.length - 1 ? ClipboardCheck : ChevronRight}>{step === steps.length - 1 ? "Criar OS mockada" : "Continuar"}</PrimaryCTA></div></div>}
  </AppShell>;
}
