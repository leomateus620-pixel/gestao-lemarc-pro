...
import { Activity, Plus, Search, UsersRound } from "lucide-react";
    ...
    <main className="mx-auto max-w-6xl space-y-4 pb-6">
      <section className="lemarc-wizard-card p-4 sm:p-5">
        <div className="grid grid-cols-[minmax(0,1fr)_auto] items-start gap-3 sm:items-end">
          <div className="min-w-0">
            ...
            <p className="mt-1 max-w-2xl text-[13px] font-medium leading-snug text-slate-300">
              Equipe técnica, valores/hora e histórico operacional real.
            </p>
          </div>
          <Link
        ...
        <div className="mt-3.5 flex gap-2 overflow-x-auto pb-1 lemarc-smart-scroll">
          ...
          <Kpi
            label="Horas mês (real)"
            value={formatMinutes(dashboard.kpis.hoursMonthRealMinutes)}
            hint={
              dashboard.kpis.hoursMonthEstimatedMinutes > 0
                ? `+${formatMinutes(dashboard.kpis.hoursMonthEstimatedMinutes)} estimado`
                : undefined
            }
          />
          <Kpi
            label="Mão de obra mês"
            value={formatMoneyOrZero(dashboard.kpis.valueMonthCentsReal)}
            hint={
              dashboard.kpis.valueMonthCentsEstimated > 0
                ? `Estimado: ${formatMoneyOrZero(dashboard.kpis.valueMonthCentsEstimated)}`
                : undefined
            }
          />
        </div>
...
function ModuleTabs() {
  ...
  return (
    <nav className="lemarc-operational-tabs" aria-label="Navegação de colaboradores">
      <ModuleTab to="/colaboradores" active={pathname === "/colaboradores"}>
        Visão geral
      </ModuleTab>
      <ModuleTab
        to="/colaboradores/novo"
        active={pathname.startsWith("/colaboradores/novo")}
      >
        Novo colaborador
      </ModuleTab>
    </nav>
...
function ModuleTab({
  ...
  return (
    <Link
      to={to as never}
      className={cn(
        "lemarc-pressable inline-flex min-h-10 shrink-0 items-center rounded-full border px-3.5 text-[11px] font-black uppercase tracking-[0.1em] transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/70",
        active
